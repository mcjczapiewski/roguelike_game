...
characters = {
    "hero": {
        ...
        "chances critical hit": 3,  # podczas ataku losujemy, aż będzie max atak
        "inventory": [],
        "points": 0,
        "print_character": "@",
    },
...
# chwilowo dla bohatera pod zmienną podstawiamy określonego wroga, który traci
# życie podczas walki tak, żeby wiele razy wygrywać z tym samym wrogiem.
def fight(enemy, mobs_on_board, enemy_position, board):
    """ initiates fight with the enemy
    enemy: dictionary with enemy characteristics
    (taken from the characters dict)
    """
    # zwroty przy ataku
    hit_words = [
        "a masz",
        "osz ty koczkodanie złap to",
        "teraz obroń się przed tym",
        "trzeba było ze mną nie zaczynać",
        "osz ty łobuzie, nie uciekniesz mi",
    ]
    ...
    offence_words = [
        ...
        "a to za co",
        "nie po twarzy",
        "zaraz ci oddam bambaryło",
        "tylko na tyle cie stać?",
        "a to za co",
        "ajjj.. znowu w klejnoty",
    ]
    ...
    check_value = ui.display_fight(
        "Ktoś tu jak zwykle szuka zaczepki..."
    ...
    while check_value:
        ...
        hero_random = characters["hero"][
            "attack"
        ]  # zapisuje max atak bohatera
        ...
        if enemy_live < 1:  # jeżeli wróg przegra
            ...
            if hero_add < 4:
                ...
                characters["hero"]["attack"] = (
                    characters["hero"]["attack"] + point
                )
                characters["hero"]["points"] = characters["hero"]["points"] + 1
            if hero_add > 5:
                outcome = "Brawo, rośnie Ci szansa uderzenia krytycznego"
                characters["hero"]["chances critical hit"] = (
                    characters["hero"]["chances critical hit"] + 1
                )
                characters["hero"]["points"] = characters["hero"]["points"] + 1
            if hero_add == 4:
                outcome = (
                    "Brawo, rośnie Ci szansa uderzenia krytycznego i atak"
                )
                characters["hero"]["chances critical hit"] = (
                    characters["hero"]["chances critical hit"] + 1
                )
            ...
            if hero_add == 5:
                ...
                characters["hero"]["points"] = (
                    characters["hero"]["points"] + point
                )
            # kończe walke
            ui.display_fight(
                "Ojoj, " + enemy_name + " już się nie rusza.\n\t" + outcome,
                enemy,
            )
            check_value = False
            fight_won = True
            ...
            if enemy_name == "madka z horom curkom":
                win_text = (
                    ...
                    + "Pokonałeś głównego bohatera, Madke z Horom curkom, \
teraz wygrywasz"
                )
                end_game(win_text)
        ...
        if enemy_live > 0:
            ...
            characters["hero"]["live"] = (
                characters["hero"]["live"] - enemy_attack
            )
            check_value = ui.display_fight(
    ...
    if fight_won:
        ...
        # teraz w zależności którą torbę wylosuje, dostanie tyle naklejek
        # na świeżaki/słodziaki
        if inventory_chosen == "laska":
            add_attack = random.randint(5, 10)
            ui.display_fight(
                "podniosłeś laskę, dostajesz dodatkowy atak w ilości "
                + str(add_attack),
                enemy,
            )
            characters["hero"]["attack"] = (
                characters["hero"]["attack"] + add_attack
            )
        if inventory_chosen == "glosnik blutuf":
            ui.display_fight(inventory_chosen + ", a na kij mi to?", enemy)
        if (
            inventory_chosen == "wyzwiska"
            or inventory_chosen == "bluzgi"
            or inventory_chosen == "mandat"
        ):
            ui.display_fight("A to %^$$%*^@# jeden", enemy)
            if inventory_chosen == "mandat":
                ui.display_fight(
                    "Ten #^%&^* chciał mi wlepić mandat. Dobrze mu tak", enemy
                )
        ...
        if inventory_chosen == "czipsy" or inventory_chosen == "pączek":
            ...
            ui.display_fight(
                "O! "
                + inventory_chosen
                + " Tego mi było trzeba, czuje się "
                + str(add_health)
                + " razy lepiej",
                enemy,
            )
            characters["hero"]["live"] = (
                characters["hero"]["live"] + add_health
            )
        if inventory_chosen == "torebka":
            ...
            ui.display_fight(
                "Teraz mam już "
                + str(characters["hero"]["live"])
                + " zdrowia"
                + "\n\t  A co tu mi wypadło? Naklejki ze świeżakami, aż 5! ",
                enemy,
            )
            characters["hero"]["points"] = characters["hero"]["points"] + 5
        if inventory_chosen == "teczka":
            ...
            ui.display_fight(
                ...
                + str(characters["hero"]["live"])
                + " zdrowia"
                + "\n\t  A co tu mi wypadło? Naklejki ze świeżakami, aż 10! ",
                enemy,
            )
        ...
        if inventory_chosen == "torebka podróbka":
            ...
            ui.display_fight(
                ...
                + str(characters["hero"]["live"])
                + " zdrowia"
                + "\n\t  A co tu mi wypadło? Naklejki ze świeżakami, aż 15! ",
                enemy,
            )
        ...
        if inventory_chosen == "plecak":
            ui.display_fight(
                "O! "
                + inventory_chosen
                + " Co my tu mamy w środku? Skąd gówniaki biorą tyle piwa? \
Nie ważne."
                + "\n\t  Tego mi było trzeba, czuje się 5 razy lepiej",
                enemy,
            )
            ...
            ui.display_fight(
                ...
                + str(characters["hero"]["live"])
                + " zdrowia"
                + "\n\t  A co tu mi wypadło? Naklejki ze świeżakami, całe 5! ",
                enemy,
            )
            characters["hero"]["points"] = characters["hero"]["points"] + 15
...
def friend_meet(friends_on_board, friend_position, board):
    friend_hello = [
        ...
        "Ooo... Siema, co tam u Ciebie",
        "Witam, co podać, w czym pomóc?",
        "czego mi tu...",
...
\n ja nic nie robiłem. \n Co chcesz?",
        "Nawet nie można spokojnie zapalić jointa w pracy...\
\n Co jest ziomek?",
        "Ja ni panimaju, cio ty ciesz?",
        "Tylko szybko, mów co chcesz",
    ]
    friend_bye = [
        "wybacz teraz, trochę się spieszę, poszukaj kogoś innego",
...
\n Idę zapalić, jak chcesz choć ze mną",
        ...
        "O rany mam już dosyć tej parszywej roboty, \
idę pouczyć się programowania",
...
\n Poczekaj, gdzie idziesz?"
        "A teraz idź mi stąd łachmaniarzu, to sklep dla porządnych ludzi, \
madek z dziećmi i emerytów",
    ]

    friend_text = random.choice(friend_hello)
    ui.display_meets(
        "Spotkałeś pracownika obsługi Biedronki\n\n\n" + friend_text
    )
    ...
    if hero_add > 5:
        ...
        characters["hero"]["chances critical hit"] = (
            characters["hero"]["chances critical hit"] + point
        )
    if hero_add == 4:
        outcome = (
            " Jedyne, co mogę Ci dac to szansa uderzenia krytycznego i atak"
        )
        characters["hero"]["chances critical hit"] = (
            characters["hero"]["chances critical hit"] + point
        )
    ...
    if hero_add == 5:
        ...
        characters["hero"]["live"] = characters["hero"]["live"] + 3 * point
    if hero_add == 7:
